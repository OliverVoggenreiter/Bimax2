...
/**
 * The class is used for storing the indices that are computed in the method selectPivot(..).
 */
public class Pivot {
	...
	/**
	 * Minimum number of disconnections that a node in X u P has. The corresponding node will be chosen as pivot. The disconnections are
	 * computed with respect to the nodes P.
	 */
	public int minDisconnections = MAX_DISCONNECTIONS;

	/**
	 * The node that is first extended and then added to X. If the pivot is from P, the selectedNode is the pivot itself, and afterwards the
	 * selectedNode is initialized iteratively with all the nodes in P not connected with the pivot. If the pivot is from X, the
	 * selectedNode will be chosen only from the set of nodes in P not connected with the pivot.
	 */
	public int selectedNode = -1;

	/**
	 * The node from X u P with the most connections with the candidate nodes. (equivalent with the node with the least discontinuities with
	 * the candidates nodes)
	 */
	public int pivot = -1;

	/**
	 * The variable can have the value 0 or 1. If the value is 0, the pivot was chosen from the X. If the value is 1, the pivot was chosen
	 * from P and needs to be first added to be first extended and then added to set X.
	 */
	 ...
	 */
	public void findPivot(Nodes setPossiblePivots, Nodes setCheckDisconnections, BinaryMatrix preprocessedMatrix) {
		int count;
		int pos = 0;
		int p;
		...
		for (int i = 0; i < setPossiblePivots.candidatesEnd && minDisconnections != 0; i++) {
			p = setPossiblePivots.nodes[i];
			count = 0;
			// find the disconections in P cols
			for (int j = setCheckDisconnections.notEnd; j < setCheckDisconnections.candidatesEnd && count < minDisconnections; j++) {
				// if it is a row
				if (pivotIsRow) {
